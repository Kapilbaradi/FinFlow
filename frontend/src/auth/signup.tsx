...
const SignUp = () => {
  ...
  const [error, setError] = useState([
    { errorField: "", errorStatus: false, errorMessage: "" },
  ]);
  ...
  const validate = (currentStep: number) => {
    ...
    switch (currentStep) {
      case 1:
        isEmail = email !== null ? emailRegex.test(email as string) : false;
        isUsername = username != null && username.length > 3 ? true : false;
  ...
  const nextStep = () => {
    ...
    if (step == 1) {
      if (!isValide?.isEmail) {
        ...
        console.log(isValide?.isEmail);
      }
      if (!isValide?.isUsername) {
        error.push(...error, {
          errorField: "username",
          errorStatus: true,
          errorMessage: "Username should of atleast 4 letters",
        });
        console.log(isValide?.isUsername);
      }
    }
