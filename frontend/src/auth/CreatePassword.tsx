import { ReactElement, useState } from "react";
import AuthWrapper from "./AuthWrapper";

interface PasswordComponentPropeType {
  showPassword: boolean;
  setShowPassword: React.Dispatch<React.SetStateAction<boolean>>;
  placeholder: string;
}

const PasswordComponent = ({
  showPassword,
  setShowPassword,
  placeholder,
}: PasswordComponentPropeType): ReactElement => {
  return (
    <>
      <div className="mb-3">
        <div className="w-full px-4 py-2 bg-[#F3F4F6FF] flex items-center rounded-2xl border border-red-500">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            strokeWidth="1.5"
            stroke="currentColor"
            className="size-5"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              d="M16.5 10.5V6.75a4.5 4.5 0 1 0-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 0 0 2.25-2.25v-6.75a2.25 2.25 0 0 0-2.25-2.25H6.75a2.25 2.25 0 0 0-2.25 2.25v6.75a2.25 2.25 0 0 0 2.25 2.25Z"
            />
          </svg>
          {showPassword ? (
            <>
              <input
                type="text"
                placeholder={placeholder}
                className="w-full px-2 bg-transparent border-0 outline-none text-base"
                name={placeholder}
              />
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth="1.5"
                stroke="currentColor"
                className="size-5 cursor-pointer"
                onClick={() => setShowPassword((prev) => !prev)}
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M3.98 8.223A10.477 10.477 0 0 0 1.934 12C3.226 16.338 7.244 19.5 12 19.5c.993 0 1.953-.138 2.863-.395M6.228 6.228A10.451 10.451 0 0 1 12 4.5c4.756 0 8.773 3.162 10.065 7.498a10.522 10.522 0 0 1-4.293 5.774M6.228 6.228 3 3m3.228 3.228 3.65 3.65m7.894 7.894L21 21m-3.228-3.228-3.65-3.65m0 0a3 3 0 1 0-4.243-4.243m4.242 4.242L9.88 9.88"
                />
              </svg>
            </>
          ) : (
            <>
              <input
                type="password"
                placeholder={placeholder}
                className="w-full px-2 bg-transparent border-0 outline-none text-base"
                name={placeholder}
              />
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth="1.5"
                stroke="currentColor"
                className="size-5 cursor-pointer"
                onClick={() => setShowPassword((prev) => !prev)}
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z"
                />
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"
                />
              </svg>
            </>
          )}
        </div>
        <p className="text-red-600 text-sm font-semibold text-left ms-2">
          Password should contain atleast 8 letters
        </p>
      </div>
    </>
  );
};

const CreatePassword = () => {
  const [showPassword, setShowPassword] = useState<boolean>(false);
  const [showConfirmPassword, setShowConfimePassword] =
    useState<boolean>(false);
  return (
    <AuthWrapper
      header="Create Password For Your Account"
      description="Enter Password"
      buttonText="Confirm Password"
    >
      <div className="w-full xl:basis-1/2 justify-end gap-2">
        <PasswordComponent
          showPassword={showPassword}
          setShowPassword={setShowPassword}
          placeholder="New Password"
        />
        <PasswordComponent
          showPassword={showConfirmPassword}
          setShowPassword={setShowConfimePassword}
          placeholder="Confirm Password"
        />
      </div>
    </AuthWrapper>
  );
};

export default CreatePassword;
